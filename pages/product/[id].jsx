...
import { addProduct } from "@/components/product/redux/cartSlice";
import { useDispatch, useSelector } from "react-redux";
import axios from "axios";

const Index = ({ food }) => {
  // Loading kontrolü ekle
  if (!food) {
    return (
      <div className="flex justify-center items-center min-h-[calc(100vh_-_88px)]">
  ...
  const [price, setPrice] = useState(food.prices?.[0] || 0);
  ...
  return (
    <div className="flex items-center md:h-[calc(100vh_-_88px)] gap-5 py-20 flex-wrap ">
      <div className="relative md:flex-1 md:w-[80%] md:h-[80%] w-36 h-36 mx-auto">
        <Image src={food?.img || "/images/default.jpg"} alt="" layout="fill" objectFit="contain" />
      </div>
      <div className="md:flex-1 md:text-start text-center">
        <Title addClass="text-6xl">{food?.title || "Product"}</Title>
